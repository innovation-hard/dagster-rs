...
@multi_asset(
    ...
    outs={
        "preprocessed_training_data": AssetOut(description="Preprocessed training data"),
        "user2Idx": AssetOut(),
        "movie2Idx": AssetOut(),
    }
)
...
def keras_dot_product_model(context, X_train, y_train, user2Idx, movie2Idx):
    ...
    model.compile(adam_v2.Adam(learning_rate=learning_rate), 'mean_squared_error')
    
    context.log.info(f'batch_size: {batch_size} - epochs: {epochs}')
    history = model.fit(
        [
            tf.convert_to_tensor(X_train.encoded_user_id.values).numpy(),
            tf.convert_to_tensor(X_train.encoded_movie_id.values).numpy()
        ], 
        tf.convert_to_tensor(y_train.rating.values).numpy(),
